...
q1 = "update order_details set total_price=unit_price*quantity;" + "update order_details set discounted_price=unit_price*quantity*(1-discount);"+"select order_id, total_price, discounted_price from order_details order by discounted_price desc limit 10;"
cursor.execute(q1)
...
q7 = "(select product_name as Product_Names, unit_price as Prices from products order by Prices desc limit 5) union all (select product_name as Product_Names, unit_price as Prices from products order by Prices asc limit 5);"
cursor.execute(q7)
rows = cursor.fetchall()
...
q8="update order_details set category_sales = unit_price*quantity*(1-discount); select extract(year from orders.shipped_date)::numeric as shippedyear, categories.category_name, round(sum(order_details.category_sales),2) as category_sales from  order_details right join orders on orders.order_id = order_details.order_id right join products on  order_details.product_id = products.product_id right join categories on categories.category_id = products.category_id where orders.shipped_date>'1997-05-31' group by category_name, shippedyear order by shippedyear;"
cursor.execute(q8)
rows = cursor.fetchall()
